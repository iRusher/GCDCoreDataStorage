...
@implementation GCDCoreDataStorage
...
- (NSManagedObjectModel *)managedObjectModel
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		DDLogVerbose(@"%@: Creating managedObjectModel (%@)", [self class], momName);
		...
		{
			DDLogWarn(@"%@: Couldn't find managedObjectModel file - %@", [self class], momName);
		}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		DDLogVerbose(@"%@: Creating persistentStoreCoordinator", [self class]);
		...
		if (databaseFileName)
		{
			...
			{
				DDLogWarn(@"%@: Error creating persistentStoreCoordinator - Nil persistentStoreDirectory",
							[self class]);
			}
		}
		else
...
- (NSManagedObjectContext *)managedObjectContext
{
	...
	if (coordinator)
	{
		DDLogVerbose(@"%@: Creating managedObjectContext", [self class]);
...
- (NSManagedObjectContext *)mainThreadManagedObjectContext
{
	...
	if (coordinator)
	{
		DDLogVerbose(@"%@: Creating mainThreadManagedObjectContext", [self class]);
...
- (void)managedObjectContextDidSave:(NSNotification *)notification
{
	...
	{
		DDLogVerbose(@"%@: %@ - Merging changes into mainThreadManagedObjectContext", THIS_FILE, THIS_METHOD);
...
- (void)save
{
	...
	{
		DDLogWarn(@"%@: Error saving - %@ %@", [self class], error, [error userInfo]);
...
- (void)maybeSave:(int32_t)currentPendingRequests
{
	...
	if ([[self managedObjectContext] hasChanges])
	{
		...
		{
			DDLogVerbose(@"%@: Triggering save (pendingRequests=%i)", [self class], currentPendingRequests);
		...
		else
		{
			...
			{
				DDLogVerbose(@"%@: Triggering save (unsavedCount=%lu)", [self class], (unsigned long)unsavedCount);
